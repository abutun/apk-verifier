 ...
 *
 * <p>
 * The engine is stateful and thus cannot be used for signing multiple APKs. However, once the
 * engine signed an APK, the engine can be used to re-sign the APK after it has been modified. This
 * may be more efficient than signing the APK using a new instance of the engine. See
 ...
 *
 * <p>
 * The input APK may contain JAR entries which, depending on the engine's configuration, may or may
 * not be output (e.g., existing signatures may need to be preserved or stripped) or which the
 ...
 * invoke {@link #outputZipSections(DataSource, DataSource, DataSource)} which may request that an
 * APK Signature Block is inserted before the ZIP Central Directory. The block contains the output
 * APK's APK Signature Scheme v2 signature.</li>
 ...
 *
 * <p>
 * <h3 id="incremental">Incremental Operation</h3></a>
 *
 ...
 * {@link #inputJarEntryRemoved(String)}, {@link #outputJarEntry(String)}, and
 * {@link #outputJarEntryRemoved(String)}, perform the tasks requested by the engine through these
 * methods, and, when a new signed APK is desired, run through steps 5 onwards to re-sign the APK.
 *
 * <p>
 * <h3>Output-only Operation</h3>
 *
...
public interface ApkSignerEngine extends Closeable {
	 ...
	 *            APK signing block of the input APK. The provided data source is guaranteed to not
	 *            be used by the engine after this method terminates.
	 *
	 ...
	 *             if a signature could not be generated because a signing key is not suitable for
	 *             generating the signature
	 * @throws SignatureException
	 *             if an error occurred while generating the JAR signature
	 * @throws IllegalStateException
	 *             if there are unfulfilled requests, such as to inspect some JAR entries, or if the
	 *             engine is closed
	 */
	 ...
	 *
	 * @param zipEntries
	 *            the section of ZIP archive containing Local File Header records and data of the
	 *            ZIP entries. In a well-formed archive, this section starts at the start of the
	 *            archive and extends all the way to the ZIP Central Directory.
	 * @param zipCentralDirectory
	 *            ZIP Central Directory section
	 * @param zipEocd
	 ...
	 *
	 * @throws IOException
	 *             if an I/O error occurs while reading the provided ZIP sections
	 * @throws InvalidKeyException
	 *             if a signature could not be generated because a signing key is not suitable for
	 *             generating the signature
	 * @throws SignatureException
	 *             if an error occurred while generating the APK's signature
	 * @throws IllegalStateException
	 *             if there are unfulfilled requests, such as to inspect some JAR entries or to
	 *             output JAR signature, or if the engine is closed
	 */
	OutputApkSigningBlockRequest outputZipSections(DataSource zipEntries, DataSource zipCentralDirectory, DataSource zipEocd) throws IOException, InvalidKeyException, SignatureException;
	 ...
	 * @throws IllegalStateException
	 *             if there are unfulfilled requests, such as to inspect some JAR entries or to
	 *             output signatures, or if the engine is closed
	 */
	 ...
	 *
	 * <p>
	 * This does not change the output APK. For example, if the output APK is not yet fully signed,
	 * it will remain so after this method terminates.
	 */
	@Override
	void close();
	...
	public static class InputJarEntryInstructions {
		 ...
		 *
		 * @param inspectJarEntryRequest
		 *            request to inspect the entry or {@code null} if there's no need to inspect the
		 *            entry.
		 */
	...
	interface OutputJarSignatureRequest {
		...
		public static class JarEntry {
			 ...
			 *
			 * @param data
			 *            uncompressed data of the entry. Changes to this array will not be
