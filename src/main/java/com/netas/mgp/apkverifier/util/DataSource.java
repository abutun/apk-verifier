 ...
 * <ul>
 * <li>Stream the chunk's data into a {@link DataSink} using {@link #feed(long, long, DataSink)
 * feed}. This is best suited for scenarios where there is no need to have the chunk's data
 * accessible at the same time, for example, when computing the digest of the chunk. If you need to
 * keep the chunk's data around after {@code feed} completes, you must create a copy during
 * {@code feed}. However, in that case the following methods of obtaining the chunk's data may be
 * more appropriate.</li>
 * <li>Obtain a {@link ByteBuffer} containing the chunk's data using
 * {@link #getByteBuffer(long, int) getByteBuffer}. Depending on the data source, the chunk's data
 * may or may not be copied by this operation. This is best suited for scenarios where you need to
 * access the chunk's data in arbitrary order, but don't need to modify the data and thus don't
 * require a copy of the data.</li>
 * <li>Copy the chunk's data to a {@link ByteBuffer} using {@link #copyTo(long, int, ByteBuffer)
 * copyTo}. This is best suited for scenarios where you require a copy of the chunk's data, such as
 * to when you need to modify the data.</li>
 * </ul>
 */
public interface DataSource {
	 ...
	 *
	 * @param offset
	 *            index (in bytes) at which the chunk starts inside data source
	 * @param size
	 ...
	 *
	 * <p>
	 * The returned buffer's position is {@code 0}, and the buffer's limit and capacity is
	 * {@code size}.
	 *
	 * @param offset
	 *            index (in bytes) at which the chunk starts inside data source
	 ...
	 *
	 * @param offset
	 *            index (in bytes) at which the region starts inside data source
	 * @param size
