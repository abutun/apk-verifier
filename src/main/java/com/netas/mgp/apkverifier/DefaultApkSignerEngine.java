...
package com.netas.mgp.apkverifier;
...
import com.netas.mgp.apkverifier.internal.apk.v1.DigestAlgorithm;
import com.netas.mgp.apkverifier.internal.apk.v1.V1SchemeSigner;
import com.netas.mgp.apkverifier.internal.apk.v2.V2SchemeSigner;
import com.netas.mgp.apkverifier.internal.util.ByteArrayOutputStreamSink;
import com.netas.mgp.apkverifier.internal.util.MessageDigestSink;
import com.netas.mgp.apkverifier.internal.util.Pair;
import com.netas.mgp.apkverifier.util.DataSink;
import com.netas.mgp.apkverifier.util.DataSource;
...
public class DefaultApkSignerEngine implements ApkSignerEngine {
	...
	private final Map<String, GetJarEntryDataDigestRequest> mOutputJarEntryDigestRequests = new HashMap<>();
	...
	private final Map<String, GetJarEntryDataRequest> mOutputSignatureJarEntryDataRequests = new HashMap<>();
	/**
	...
	private DefaultApkSignerEngine(List<SignerConfig> signerConfigs, int minSdkVersion, boolean v1SigningEnabled, boolean v2SigningEnabled, boolean otherSignersSignaturesPreserved) throws InvalidKeyException {
		...
		mV1SignerConfigs = (v1SigningEnabled) ? new ArrayList<>(signerConfigs.size()) : Collections.emptyList();
		mV2SignerConfigs = (v2SigningEnabled) ? new ArrayList<>(signerConfigs.size()) : Collections.emptyList();
		mV1ContentDigestAlgorithm = (v1SigningEnabled) ? V1SchemeSigner.getSuggestedContentDigestAlgorithm(minSdkVersion) : null;
		for (SignerConfig signerConfig : signerConfigs) {
			...
			if (v2SigningEnabled) {
				...
				v2SignerConfig.signatureAlgorithms = V2SchemeSigner.getSuggestedSignatureAlgorithms(publicKey, minSdkVersion);
				mV2SignerConfigs.add(v2SignerConfig);
			}
		}
		mSignatureExpectedOutputJarEntryNames = (v1SigningEnabled) ? V1SchemeSigner.getOutputEntryNames(mV1SignerConfigs) : Collections.emptySet();
	}
	...
	public InputJarEntryInstructions inputJarEntry(String entryName) {
		...
		InputJarEntryInstructions.OutputPolicy outputPolicy = getInputJarEntryOutputPolicy(entryName);
		switch (outputPolicy) {
		case SKIP:
		...
		case OUTPUT_BY_ENGINE:
			if (V1SchemeSigner.MANIFEST_ENTRY_NAME.equals(entryName)) {
				...
				return new InputJarEntryInstructions(InputJarEntryInstructions.OutputPolicy.OUTPUT_BY_ENGINE, mInputJarManifestEntryDataRequest);
			}
			return new InputJarEntryInstructions(InputJarEntryInstructions.OutputPolicy.OUTPUT_BY_ENGINE);
		default:
	...
	public InspectJarEntryRequest outputJarEntry(String entryName) {
		...
		if (V1SchemeSigner.isJarEntryDigestNeededInManifest(entryName)) {
			...
			GetJarEntryDataDigestRequest dataDigestRequest = new GetJarEntryDataDigestRequest(entryName, V1SchemeSigner.getJcaMessageDigestAlgorithm(mV1ContentDigestAlgorithm));
		...
		if (mSignatureExpectedOutputJarEntryNames.contains(entryName)) {
			...
			if (V1SchemeSigner.MANIFEST_ENTRY_NAME.equals(entryName)) {
				...
				dataRequest = (mEmittedSignatureJarEntryData.containsKey(entryName)) ? new GetJarEntryDataRequest(entryName) : null;
			}
	...
	@Override
	public OutputApkSigningBlockRequest outputZipSections(DataSource zipEntries, DataSource zipCentralDirectory, DataSource zipEocd) throws IOException, InvalidKeyException, SignatureException {
		checkNotClosed();
	...
	private InputJarEntryInstructions.OutputPolicy getInputJarEntryOutputPolicy(String entryName) {
		...
		if ((mOtherSignersSignaturesPreserved) || (V1SchemeSigner.isJarEntryDigestNeededInManifest(entryName))) {
	...
	private static class OutputJarSignatureRequestImpl implements OutputJarSignatureRequest {
		...
		private OutputJarSignatureRequestImpl(List<JarEntry> additionalZipEntries) {
			mAdditionalJarEntries = Collections.unmodifiableList(new ArrayList<>(additionalZipEntries));
		}
			...
			synchronized (mLock) {
				if (mMessageDigest == null) {
					try {
						...
						throw new RuntimeException(mJcaDigestAlgorithm + " MessageDigest not available", e);
					}
				}
	...
	public static class SignerConfig {
		...
		public static class Builder {
			 ...
			 *            signing key
			 * @param certificates
			 *            list of one or more X.509 certificates. The subject public key of the
	...
	public static class Builder {
		 ...
		 *
		 * @param signerConfigs
		 *            information about signers with which the APK will be signed. At least one
		 *            signer configuration must be provided.
		 * @param minSdkVersion
		 *            API Level of the oldest Android platform on which the APK is supposed to be
		 *            installed. See {@code minSdkVersion} attribute in the APK's
		 *            {@code AndroidManifest.xml}. The higher the version, the stronger signing
		...
		public DefaultApkSignerEngine build() throws InvalidKeyException {
			return new DefaultApkSignerEngine(mSignerConfigs, mMinSdkVersion, mV1SigningEnabled, mV2SigningEnabled, mOtherSignersSignaturesPreserved);
		}
