...
import com.netas.mgp.apkverifier.ApkVerifier.Issue;
import com.netas.mgp.apkverifier.ApkVerifier.IssueWithParams;
import com.netas.mgp.apkverifier.apk.ApkUtils;
import com.netas.mgp.apkverifier.internal.util.ByteBufferDataSource;
import com.netas.mgp.apkverifier.internal.util.DelegatingX509Certificate;
...
public abstract class V2SchemeVerifier {
	 ...
	 *             if no APK Signature Scheme v2 signatures are found
	 * @throws IOException
	 *             if an I/O error occurs when reading the APK
	 */
	 ...
	 * {@code result}.
	 */
	private static void verify(DataSource beforeApkSigningBlock, ByteBuffer apkSignatureSchemeV2Block, DataSource centralDir, ByteBuffer eocd, Result result) throws IOException {
	 ...
	 *
	 * <p>
	 * This verifies signatures over {@code signed-data} contained in this block but does not verify
	 * the integrity of the rest of the APK. Rather, this method adds to the
	...
	private static List<SupportedSignature> getSignaturesToVerify(List<SupportedSignature> signatures) {
		...
		for (SupportedSignature sig : signatures) {
			...
			if ((bestSigAlgorithm == null) || (compareSignatureAlgorithm(sigAlgorithm, bestSigAlgorithm) > 0)) {
	 ...
	 * @throws IOException
	 *             if an I/O error occurs while reading the APK
	 */
	private static SignatureInfo findSignature(DataSource apk, ApkUtils.ZipSections zipSections, Result result) throws IOException, SignatureNotFoundException {
		...
		Pair<ByteBuffer, Long> apkSigningBlockAndOffset = findApkSigningBlock(apk, centralDirStartOffset);
		...
		ByteBuffer apkSignatureSchemeV2Block = findApkSignatureSchemeV2Block(apkSigningBlock, result);

		return new SignatureInfo(apkSignatureSchemeV2Block, apkSigningBlockOffset, centralDirStartOffset, eocdStartOffset, eocd);
	}

	private static Pair<ByteBuffer, Long> findApkSigningBlock(DataSource apk, long centralDirOffset) throws IOException, SignatureNotFoundException {
		// FORMAT:
	 ...
	 *
	 * <p>
	 * This method reads the next {@code size} bytes at this buffer's current position, returning
	 * them as a {@code ByteBuffer} with start set to 0, limit and capacity set to {@code size},
	 * byte order set to this buffer's byte order; and then increments the position by {@code size}.
	 */
