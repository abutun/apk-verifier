...
package com.netas.mgp.apkverifier.internal.util;
...
public class ByteBufferDataSource implements DataSource {
	...
	public ByteBufferDataSource slice(long offset, long size) {
		...
		return new ByteBufferDataSource(getByteBuffer(offset, (int) size), false // no need to slice
																					// -- it's
																					// already a
																					// slice
		);
	...
	private void checkChunkValid(long offset, long size) {
		...
		if (offset > mSize) {
			throw new IllegalArgumentException("offset (" + offset + ") > source size (" + mSize + ")");
		}
		long endOffset = offset + size;
		if (endOffset < offset) {
			throw new IllegalArgumentException("offset (" + offset + ") + size (" + size + ") overflow");
		}
		if (endOffset > mSize) {
