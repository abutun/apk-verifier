...
package com.netas.mgp.apkverifier.internal.zip;
 ...
 *
 * <p>
 * NOTE: Most helper methods operating on {@code ByteBuffer} instances expect that the byte order of
 * these buffers is little-endian.
 */
public abstract class ZipUtils {
	...
	public static int getZipEocdCentralDirectoryTotalRecordCount(ByteBuffer zipEndOfCentralDirectory) {
		assertByteOrderLittleEndian(zipEndOfCentralDirectory);
		return getUnsignedInt16(zipEndOfCentralDirectory, zipEndOfCentralDirectory.position() + ZIP_EOCD_CENTRAL_DIR_TOTAL_RECORD_COUNT_OFFSET);
	}
	 ...
	 * Returns the ZIP End of Central Directory record of the provided ZIP file.
	 *
	 * @param maxCommentSize
	 *            maximum accepted size (in bytes) of EoCD comment field. The permitted value is
	 *            from 0 to 65535 inclusive. The smaller the value, the faster this method locates
	 ...
	 *
	 * @param zipEndOfCentralDirectoryPosition
	 *            offset of the ZIP End of Central Directory record in the file.
	 *
	 * @throws IOException
	 *             if an I/O error occurs while reading the data source
	 */
