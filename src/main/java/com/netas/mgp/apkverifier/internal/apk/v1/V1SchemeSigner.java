...
package com.netas.mgp.apkverifier.internal.apk.v1;
...
import com.netas.mgp.apkverifier.internal.jar.ManifestWriter;
import com.netas.mgp.apkverifier.internal.jar.SignatureFileWriter;
 ...
 *
 * @see <a href=
 *      "https://docs.oracle.com/javase/8/docs/technotes/guides/jar/jar.html#Signed_JAR_File">Signed
 *      JAR File</a>
 */
public abstract class V1SchemeSigner {
	 ...
	 *             if the provided key is not suitable for signing APKs using JAR signing (aka v1
	 *             signature scheme)
	 */
	...
	public static boolean isJarEntryDigestNeededInManifest(String entryName) {
		...
		if (("manifest.mf".equals(fileNameLowerCase)) || (fileNameLowerCase.endsWith(".sf")) || (fileNameLowerCase.endsWith(".rsa")) || (fileNameLowerCase.endsWith(".dsa")) || (fileNameLowerCase.endsWith(".ec"))
	 ...
	 */
	public static List<Pair<String, byte[]>> sign(List<SignerConfig> signerConfigs, DigestAlgorithm jarEntryDigestAlgorithm, Map<String, byte[]> jarEntryDigests, List<Integer> apkSigningSchemeIds,
			byte[] sourceManifestBytes) throws InvalidKeyException, CertificateEncodingException, SignatureException {
		if (signerConfigs.isEmpty()) {
			throw new IllegalArgumentException("At least one signer config must be provided");
		}
		OutputManifestFile manifest = generateManifestFile(jarEntryDigestAlgorithm, jarEntryDigests, sourceManifestBytes);
	 ...
	 *             if an error occurs when computing digests of generating signatures
	 */
	public static List<Pair<String, byte[]>> signManifest(List<SignerConfig> signerConfigs, DigestAlgorithm digestAlgorithm, List<Integer> apkSigningSchemeIds, OutputManifestFile manifest)
	 ...
	 */
	private static class SignerInfoSignatureAlgorithmFinder implements CMSSignatureEncryptionAlgorithmFinder {
		private static final SignerInfoSignatureAlgorithmFinder INSTANCE = new SignerInfoSignatureAlgorithmFinder();

		private static final AlgorithmIdentifier DSA = new AlgorithmIdentifier(X9ObjectIdentifiers.id_dsa, DERNull.INSTANCE);
