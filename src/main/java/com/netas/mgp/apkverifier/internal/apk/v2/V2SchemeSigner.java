...
package com.netas.mgp.apkverifier.internal.apk.v2;
...
import com.netas.mgp.apkverifier.internal.zip.ZipUtils;
import com.netas.mgp.apkverifier.util.DataSource;
import com.netas.mgp.apkverifier.util.DataSources;

/**
 * APK Signature Scheme v2 signer.
 *
 * <p>
 * APK Signature Scheme v2 is a whole-file signature scheme which aims to protect every single bit
 * of the APK, as opposed to the JAR Signature Scheme which protects only the names and uncompressed
...
public abstract class V2SchemeSigner {
	/*
	 * The two main goals of APK Signature Scheme v2 are: 1. Detect any unauthorized modifications
	 * to the APK. This is achieved by making the signature cover every byte of the APK being
	 * signed. 2. Enable much faster signature and integrity verification. This is achieved by
	 * requiring only a minimal amount of APK parsing before the signature is verified, thus
	 * completely bypassing ZIP entry decompression and by making integrity verification
	 * parallelizable by employing a hash tree.
	 *
	...
	private static final byte[] APK_SIGNING_BLOCK_MAGIC = new byte[] { 0x41, 0x50, 0x4b, 0x20, 0x53, 0x69, 0x67, 0x20, 0x42, 0x6c, 0x6f, 0x63, 0x6b, 0x20, 0x34, 0x32, };
	 ...
	 *
	 * @param minSdkVersion
	 *            minimum API Level of the platform on which the APK may be installed (see
	 *            AndroidManifest.xml minSdkVersion attribute).
	 *
	 * @throws InvalidKeyException
	 *             if the provided key is not suitable for signing APKs using APK Signature Scheme
	 *             v2
	 */
	public static List<SignatureAlgorithm> getSuggestedSignatureAlgorithms(PublicKey signingKey, int minSdkVersion) throws InvalidKeyException {
	 ...
	 *
	 * @param signerConfigs
	 *            signer configurations, one for each signer At least one signer config must be
	 *            provided.
	 *
	 * @throws IOException
	 *             if an I/O error occurs
	 * @throws InvalidKeyException
	 *             if a signing key is not suitable for this signature scheme or cannot be used in
	 *             general
	 * @throws SignatureException
	 *             if an error occurs when computing digests of generating signatures
	 */
	...
	private static byte[] generateApkSigningBlock(List<SignerConfig> signerConfigs, Map<ContentDigestAlgorithm, byte[]> contentDigests) throws InvalidKeyException, SignatureException {
	...
	private static byte[] generateSignerBlock(SignerConfig signerConfig, Map<ContentDigestAlgorithm, byte[]> contentDigests) throws InvalidKeyException, SignatureException {
		...
		signer.signedData = encodeAsSequenceOfLengthPrefixedElements(
				new byte[][] { encodeAsSequenceOfLengthPrefixedPairsOfIntAndLengthPrefixedBytes(signedData.digests), encodeAsSequenceOfLengthPrefixedElements(signedData.certificates),
	...
	private static byte[] encodeAsSequenceOfLengthPrefixedElements(List<byte[]> sequence) {
		return encodeAsSequenceOfLengthPrefixedElements(sequence.toArray(new byte[sequence.size()][]));
	}
